...
def schedule_review_dates(study_date: datetime.date) -> list[str]:
    ...
    :param study_date: A `datetime.date` object representing the study date.
    :return: List of review dates in "YYYY-MM-DD" format.
    """
    review_dates = [
        study_date + datetime.timedelta(days=offset)
        for offset in INTERVAL_OFFSETS
    ]
    return format_dates(review_dates)

def get_study_dates(review_date: datetime.date) -> list[str]:
    """
    ...
    """
    study_dates = [
        review_date - datetime.timedelta(days=offset)
        for offset in INTERVAL_OFFSETS
    ]
    return format_dates(study_dates)

def format_dates(dates: list[datetime.date]) -> list[str]:
    """
